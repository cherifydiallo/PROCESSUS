#include "priorite.h"
#include "ui_priorite.h"

Priorite::Priorite(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Priorite)
{
    ui->setupUi(this);
...
QWidget * Priorite::le_parent_priorite()
{
    return this;
}
...
void Priorite::graphique_Priorite(QVector<QVector<int> > tab)
{
    ...
    QVector <int> l(4),elu(4);
    int i;
    l[0]=l[1]=l[2]=l[3]=0;
    elu[0]=elu[1]=elu[2]=elu[3]=0;
    ...
    for(i=0;i<tab.size();i++)
    {
     ...
     l[3]=tab[i][2];//RECUPERATION DES PRIORITES
     tab1.append(l);
    }
  ...
  sortir=0;
    ...
    while(sortir==0){
         ...
         if(tab1.size()!=0) {trier_propriete(tab1);
      ...
      positi=350/(t_elu.size()+1);
     for(int k=0;k<t_elu.size();k++){

     t_elu[k][2]=345-t_elu[k][2]*positi;
          ...
          QPen bic;
           ...
           QGraphicsScene *scene=new QGraphicsScene(le_parent_priorite());
           QGraphicsView *visualiser=new QGraphicsView(le_parent_priorite());
           visualiser->setFixedSize(600,500);
           visualiser->setScene(scene);
           bic.setColor(Qt::black);
           bic.setWidth(5);

           le_parent_priorite()->show();
...
void Priorite::trier_propriete(QVector<QVector<int> > tab)
{
    int i,j,imin;
...
void Priorite::diagramme_gant_priorite(QVector<QVector<int> > tab)
{
